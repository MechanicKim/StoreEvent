...
const URL = 'https://search.naver.com/p/csearch/content/nqapirender.nhn?pkid=465&where=m';
const Display = 20;
...
export default class Main extends Component {
  constructor(props) {
    ...
    this.state = {
      items: [], page: 0, moreOn: false,
      store:'CU', category: '음료', type: '11'
    };
  }
  ...
  requestEvent = (page, store, category, type, more) => {
    ...
    .then(json => {
      const itemsHTML = HTMLParser.parse(json.current.html).querySelectorAll('li');
  ...
  render() {
    const { items, store, category, type, moreOn } = this.state;
    ...
    return (
      <Page>
        ...
        <StoreMenu store={store} category={category} type={type}
                   selectStore={this.selectStore} selectCategory={this.selectCategory}
